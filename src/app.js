...
function render() {
  ...
  function renderWorklog() {
    const S = {
      wrap:    'display:flex;flex-direction:column;gap:0;line-height:1.55;color:#3d1f00;',
      h2:      'font-size:1.05rem;font-weight:700;color:#3d1f00;margin:18px 0 6px;border-bottom:2px solid #8b5e3c;padding-bottom:4px;',
      h3:      'font-size:.98rem;font-weight:700;color:#5c3319;margin:14px 0 5px;',
      p:       'font-size:.88rem;color:#3d1f00;margin:0 0 7px;',
      li:      'font-size:.88rem;color:#3d1f00;padding:2px 0 2px 12px;position:relative;',
      li_dot:  'position:absolute;left:0;color:#8b5e3c;',
      bold:    'font-weight:700;color:#5c3319;',
      note:    'font-size:.8rem;color:#7a5235;font-style:italic;border-left:3px solid rgba(139,94,60,.4);padding:6px 10px;margin:8px 0;background:rgba(92,51,25,.06);',
    }

    function md(text) {
      return text
        .replace(/\*\*(.+?)\*\*/g, `<strong style="${S.bold}">$1</strong>`)
        .replace(/`(.+?)`/g, `<code style="background:rgba(92,51,25,.1);padding:0 3px;border-radius:2px;font-size:.85em;">$1</code>`)
    }
    ...
    const sections = [
      {
        ...
        body: [
          { p: 'Выбрала сценарий «Ферма невидимых кроликов» — есть где показать архитектурное мышление (инференсы). Начала с разбора ТЗ: что требуется и как проектировать? ЛЛМ на рекомендации, интерфейс в игровой стилистике (задание напомнило Stardew Valley).' },
          { p: 'Формат: собственный сабдомен, деплой на свою инфраструктуру (Vultr + Caddy + GitHub Actions) — тот же паттерн, что на личных проектах. Сразу отрисовала фавикон. Клод назвал желание поднять бэкенд оверинжинирингом — потратила токены, чтобы объяснить идею. Параллельно подняла локалхост по предложению Клода — **это оказалось огромной ошибкой** и стоило мне часов времени.' },
        ],
      },
      {
        h3: '2. Структура и архитектура',
        body: [
          { p: 'Разбирали с Клодом работу модели: как след превращается в оценку кроликов, как считается уверенность, какие веса чему присваиваем. Стартовые данные из ТЗ стали тестовыми логами журнала. Процесс небыстрый — долго утверждали, что и как считать. Клод хотел упростить, мне казалось, что можно развить до вменяемой системы.' },
          { p: 'Например, почему не учитываем время перемещения в оценке модели? Добавила схлопывание сигналов по времени для правдоподобия — Клод принял и внёс в структуру.' },
          { p: '**Почему не оркестрация агентов:** хотела контролировать процесс и своевременно вносить правки. Мне в кайф, время не поджимало.' },
          { p: '**Почему не пишу промпты сама:** Клод справляется с этим быстрее и точнее. Цикл разработки: сначала каркас, потом минимальный UI и тесты, потом интеграция ЛЛМ и Docker.' },
        ],
      },
      {
        ...
        body: [
          { p: '**Решила сама:**' },
          { ul: [
            'Настояла, что модель должна учитывать время не только в схлопывании, но и в расчёте уверенности.',
            'Нашла логическую дыру в факторе «согласованность»: он считал только пространственный разброс, игнорируя время. Заставила переписать.',
            'ЛЛМ на рекомендации — Claude Haiku дешёвый и живой в таких задачах.',
            'Разделила «Журнал сигналов» и «Настройку модели». Клод сначала смешал их.',
            'Добавила кастомные типы сигналов: вес задаётся и при создании, и в настройках.',
            'Мелкие правки: округление, расположение элементов, кнопочки.',
          ]},
          { p: '**Изменила после ответа ИИ:**' },
          { ul: [
            'Скорость перемещения кроликов не как константу, а как параметр.',
            'Не делать вебхук — фича сломалась, откатила.',
            'Закрыть эндпоинт к платному API.',
            'Добавить свежесть сигнала в оценку.',
            'Переосмыслить дизайн: так появилась кнопка дневника фермера.',
          ]},
        ],
      },
      {
        ...
        body: [
          { ul: [
            'Убрала технический жаргон отовсюду — ловила на себе, что не сразу понимаю термин, значит и фермер не поймёт.',
            'Развела шапку (JS, бесплатно, обновляется на каждое движение слайдера) и вкладку рекомендаций (LLM, с фоллбэком на правилах).',
            'Заставила добавить разбор оценки понятным языком вместо сырых чисел.',
            'Сделала маленькие подсказки — лучше, чем гайд на всю страницу с занудной лексикой.',
            'Несколько раз перепроверила логику модели, даже пересчитала вручную — красивый интерфейс не имеет значения, если кор работает криво.',
            'Отрисовала дизайн в Claude: закинула ассеты из Stardew для вайба.',
          ]},
        ],
      },
      {
        ...
        body: [
          { ul: [
            '**Разработка на локалхосте** — при переносе на сервер не закоммитилась часть файлов. Стоило нескольких часов жизни и тысяч токенов.',
            'Дропдаун оставался системным дважды, несмотря на отчёт «исправлено» — фиксила и проверяла глазами каждый раз.',
            'Импорт JSON затирал журнал вместо добавления к нему. Делегировала фикс Клоду.',
            'PDF-отчёт дублировал сырой экспорт. Решила имплементировать картинки вместо текста, сознательно отказавшись от полного фикса.',
            'Кастомный тип сигнала показывал технический id вместо названия в шапке. Быстрый фикс промптом.',
            'Чуть не закоммитила .env в публичный репо — проверила git-check-ignore перед публикацией.',
            'После деплоя сломалась интеграция с Claude — проблема в инфраструктуре, подтянула не тот токен на сервере.',
            'Клод код неправильно перенёс дизайн-проект — правила долго промптами и частично вручную.',
          ]},
        ],
      },
      {
        ...
        body: [
          { note: 'Я никогда не верю ни одному текстовому отчёту на слово. Claude очень любит откладывать мелкие фиксы и/или игнорировать часть инструкций.' },
          { ul: [
            'Заложила в архитектуру автотесты.',
            'Сверила стартовые данные 1:1 с JSON из ТЗ построчно.',
            'Посчитала вклад одного сигнала вручную по формуле, сверила с UI.',
            'Проверила интеграцию с Claude напрямую через curl, а не по виду интерфейса.',
            'Прогнала деплой через реальный Docker, когда что-то не сходилось.',
            'Закинула тестовое в Клод, заставила написать чеклист — проходилась по каждому пункту и сверяла с реальным кодом.',
          ]},
          { p: '**— Карина Ларк**' },
        ],
      },
    ...
    const sectionsHtml = sections.map(({ h3, body }) => {
      const bodyHtml = body.map(block => {
        if (block.p)    return `<p style="${S.p}">${md(block.p)}</p>`
        if (block.note) return `<div style="${S.note}">${md(block.note)}</div>`
        if (block.ul)   return `<ul style="list-style:none;margin:0 0 7px;padding:0;">${block.ul.map(li =>
          `<li style="${S.li}"><span style="${S.li_dot}">·</span>${md(li)}</li>`).join('')}</ul>`
        return ''
      }).join('')
      return `<div style="margin-bottom:4px;">
        <div style="${S.h3}">${h3}</div>
        ${bodyHtml}
      </div>`
    }).join('')

    return `<div style="${S.wrap}">
      <div style="${S.h2}">🤖 AI Worklog — как разрабатывала этот проект</div>
      <p style="${S.p}">Постаралась описать кратко процесс разработки. Тестовое сделано с помощью Claude, никакие другие ЛЛМ не использовала. По всем вопросам могу ответить дополнительно :)</p>
      ${sectionsHtml}
    </div>`
  }
